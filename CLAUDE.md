...
## Operasyon Bekçisi
Sunucuda her sabah 08:30'da `runOpsWatchdog()` (server.ts) koşar: offsite yedek tazeliği (db-backups/ + uploads-backups/), Mikro sync tazeliği, stok oranı çöküşü, retry kuyruğu (`syncJobs`), kur tazeliği, bant genişliği self-testi (Cloudflare'den 5MB indir + 1MB yükle; eşik ↓512/↑256 KB/sn — 2026-07-20 sağlayıcı hat çöküşü arıza sınıfı), Gemini AI sağlığı (`ai_gemini`: günde 1 mini generateContent; anahtar+model+kota — 2026-07-20 model emekliliği arıza sınıfı). Gemini model varsayılanı `gemini-flash-latest` (env `GEMINI_MODEL` veya `settings/aiConfig.geminiModel` override); anahtar önceliği UI(settings) → env → Vertex. Sonuçlar: `opsChecks` koleksiyonu (global, TENANT dışı), Admin → süper-admin panelindeki kart, `GET /api/ops/watchdog`. FAIL görürsen önce `detail` alanını oku, kök nedeni düzelt — eşik ayarıyla susturma.
