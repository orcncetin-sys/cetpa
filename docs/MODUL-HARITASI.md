...
| `src/components/AccountingModule.tsx` + `src/components/accounting/*` (21 sekme) | ✅ 2026-09-19 | kabuk 2,859 → 2,476 (sekmeler 4,760) | 37 + 43 → **0** | 13 grup + hook → `src/utils/muhasebe/` 15 yeni modül (mizan, kdvBeyan, gelirGider, gelirTablosu, satislar, faturalar, bankaHesap, cariImport, depoDeger, isletmeSermayesi, butceGerceklesme, depoNo, mikroCiro, kpiTrend, irsaliyeCalisan) + `useMikroFaturalar.test.ts`; suite 875 → 1,282 | 116 → 26 (şablonlu; elle yazılmış `AT` + kodmod = 342 anahtar `src/i18n/accounting.ts`, `ac()`) | `useMikroFaturalar` bilinmeyen tutar/KDV/matrahı NaN verir (sunucu `ISNULL(…,0)` kalktı; 2026-09-18 öncesi import edilmiş dokümanlar bayat ₺0 taşır — koruma var, kesin çözüm yeniden import); sıralama `para.sayiSirala` (bilinmeyen sonda); form alanında boş = bilinmiyor, bilinmeyen alan DB'ye YAZILMAZ (müşteri limiti/bakiye, tedarikçi bakiye, çalışan maaşı, irsaliye toplamı); irsaliye toplamı artık kalemlerden türetilip kaydediliyor (eskiden hep `total: 0`); `shared.ts` döngüsel barrel'ı → gerçek tanımlı `shared.tsx`; DashboardPage fazına devreden: `useReportsData` / `useSekmeVerileri:134` / `BankStatementImportModal:137` `\|\| 0` siteleri |
...
| `src/components/AccountingModule.tsx` ✅ Faz 3 kapandı 2026-09-19 (A2) | 2,873 | 93 | 48 | ✓ | bankAccounts, bankTransactions, budgets, cariBalances, checks… |
