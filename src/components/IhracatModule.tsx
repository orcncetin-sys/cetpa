    ...
    <div className="space-y-6">
      ...
      {activeTab === 'ihracat' && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Toplam İhracat', value: ihracatlar.length, icon: Ship, color: 'text-brand' },
            { label: 'Aylık Tutar (USD)', value: `$${ihracatToplam.toLocaleString()}`, icon: TrendingUp, color: 'text-green-600' },
            { label: 'Bekleyen Gümrük', value: bekleyenGumruk, icon: Clock, color: 'text-amber-600' },
            { label: 'Ort. Teslimat (gün)', value: '14', icon: CheckCircle, color: 'text-blue-600' },
          ].map(kpi => (
            <div key={kpi.label} className="apple-card">
