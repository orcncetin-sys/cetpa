                ...
                <tr key={w.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="py-2.5 px-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => { setEditingWaybill(w); setWaybillForm({ waybillNo: w.waybillNo, invoiceNo: w.invoiceNo || '', party: w.party, date: w.date, items: w.items || [], status: w.status, warehouseId: w.warehouseId || '' }); setWaybillType('gelen'); setShowWaybillModal(true); }} className="p-1.5 hover:bg-blue-50 rounded-lg transition-colors text-blue-500"><Eye size={13} /></button>
