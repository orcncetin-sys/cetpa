        ...
        <>
                ...
                {p605ShowForm&&(
                  <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      <input type="number" className="apple-input px-3 py-2 text-sm" placeholder={oc(tr605).planlanan} value={p605Draft.planned} onChange={e=>setP605Draft(d=>({...d,planned:e.target.value}))} />
                      <input type="number" className="apple-input px-3 py-2 text-sm" placeholder={oc(tr605).gerceklesen} value={p605Draft.actual} onChange={e=>setP605Draft(d=>({...d,actual:e.target.value}))} />
                    </div>
                    <div className="flex gap-2">
                      <button onClick={async ()=>{if(!p605Draft.line) return; try{const payload605={line:p605Draft.line,maxCap:Number(p605Draft.maxCap)||0,planned:Number(p605Draft.planned)||0,actual:Number(p605Draft.actual)||0}; if(p605EditId){ await updateDoc(doc(db,'capacityLines',p605EditId),payload605); setP605EditId(null); } else { await addDoc(collection(db,'capacityLines'),{...payload605,createdAt:serverTimestamp()}); }}catch(e){console.error("[firestore]", e); toast(tr605?'Hat kaydedilemedi (yetki?).':'Save failed.','error');} setP605Draft({line:'',maxCap:'',planned:'',actual:''}); setP605ShowForm(false);}} className="apple-button-primary text-sm px-4 py-1.5">{oc(tr605).kaydet}</button>
                      <button onClick={()=>setP605ShowForm(false)} className="apple-button-secondary text-sm px-4 py-1.5">{oc(tr605).iptal}</button>
                    </div>
                  </div>
                )}
                      ...
                      return (
                        <div key={i} className="space-y-1.5">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-gray-500">{l.actual}/{l.maxCap} {oc(tr605).birim_2} ({actPct.toFixed(0)}%)</span>
                            {l.id&&<button onClick={()=>{setP605Draft({line:l.line,maxCap:String(l.maxCap),planned:String(l.planned),actual:String(l.actual)});setP605EditId(l.id!);setP605ShowForm(true);}} title={oc(tr605).duzenle} className="text-gray-300 hover:text-blue-600"><Edit2 className="w-3.5 h-3.5"/></button>}
                            {l.id&&<button onClick={async ()=>{try{await deleteDoc(doc(db,'capacityLines',l.id!));}catch(e){console.error("[firestore]", e); toast(oc(tr605).silinemedi_yetki,'error');}}} title={oc(tr605).sil} className="text-gray-300 hover:text-red-600"><Trash2 className="w-3.5 h-3.5"/></button>}
                            </div>
                          </div>
                          ...
                          <p className="text-[10px] text-gray-400">{oc(tr605).planlanan_2} {planPct.toFixed(0)}% · {tr605?'Gerçekleşen:':'Actual:'} {actPct.toFixed(0)}%</p>
                        </div>
                      );
                    })}
                  </div>
            ...
            return (
              <div className="apple-card p-5 space-y-4">
                ...
                {p624ShowForm && (
                  <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      <input className="apple-input col-span-2 md:col-span-1" placeholder={oc(tr624).urun} value={p624Draft.productName} onChange={e=>setP624Draft(d=>({...d,productName:e.target.value}))}/>
                      <input type="number" className="apple-input" placeholder={oc(tr624).miktar} value={p624Draft.qty} onChange={e=>setP624Draft(d=>({...d,qty:e.target.value}))}/>
                      <input className="apple-input" placeholder={oc(tr624).is_merkezi} value={p624Draft.workCenter} onChange={e=>setP624Draft(d=>({...d,workCenter:e.target.value}))}/>
                    ...
                    }} className="apple-button-primary text-xs px-6">{oc(tr624).olustur}</button>
                  </div>
                )}
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-amber-50 rounded-xl p-3"><p className="text-[10px] font-bold text-gray-400 uppercase">{oc(tr624).planlandi}</p><p className="text-xl font-black text-amber-600">{p624Orders.filter(o=>o.status==='Planlandı').length}</p></div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs min-w-[560px]">
                      <thead><tr className="border-b border-gray-100 bg-gray-50">
                        {[oc(tr624).urun,oc(tr624).miktar,oc(tr624).is_merkezi,tr624?'Başlangıç':'Start',tr624?'Bitiş':'End',oc(tr624).durum,oc(tr624).oncelik].map(h=>(
                        ...
                        {[...p624Orders].sort((a,b)=>a.plannedStart.localeCompare(b.plannedStart)).map(o=>(
                          <tr key={o.id} className={`hover:bg-gray-50/50 ${o.priority==='Acil'?'bg-red-50/20':''}`}>
                            ...
                            <td className="px-3 py-2.5">
                              <select value={o.status} onChange={async e=>{try{await updateDoc(doc(db,'productionOrders',o.id),{status:e.target.value});}catch(err){console.error(err); toast(oc(tr624).durum_guncellenemedi_yetki,'error');}}} className={`text-[10px] font-bold px-2 py-0.5 rounded-full border-0 ${statusCls[o.status]}`}>
          ...
          {(() => {
            ...
            upcoming637.forEach(o=>{
              ...
              const wc = o.workCenter || (oc(tr637).genel_hat);
                      ...
                      return (
                        <div key={r.wc}>
                          <div className="flex items-center justify-between text-xs mb-1">
                            ...
                            <span className="text-gray-500">{r.orders} {tr637?'emir':'orders'} · {r.totalQty} {oc(tr637).birim_2}</span>
                          </div>
